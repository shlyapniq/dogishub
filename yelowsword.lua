...
w1.BrickColor = BrickColor.new("Bright yellow")
w1.Transparency = 0
w1.Reflectance = 0
w1.Material = "SmoothPlastic"
w1.CanCollide = false
...
w2.BrickColor = BrickColor.new("Bright yellow")
w2.Transparency = 0
w2.Reflectance = 0
w2.Material = "SmoothPlastic"
w2.CanCollide = false
...
local Hat = AlginChar:FindFirstChild("Back_AccAccessory")
local Count = 1
...
AlginChar.Reanimate["Back_AccAccessory"].Handle.Attachment:Destroy()
Align(Hat.Handle,AlginChar.Model.Handle,Vector3.new(0, 1.3, 0),Vector3.new(-5, -4, -140))
...
for _,v in pairs(AlginChar.Model:GetChildren()) do
    if v:IsA("Part") then
        v.Transparency = 1
    end
    end
...
con1=Hitbox.Touched:connect(function(hit) Damagefunc(hit,0,0,math.random(0,0),"Normal",RootPart,.2,1) end) 
for i=1,2 do
for i=0,1,0.1 do
swait()
